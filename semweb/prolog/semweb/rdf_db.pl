...
%
% Moritz: re-activate this to handle namespace:id compounds that are packed in variables
%
rdf_has(Subject, Predicate, Object) :-
%         not(compound(Subject)),
%         not(compound(Predicate)),
%         not(compound(Object)),
        rdf_has(Subject, Predicate, Object, _).

% rdf_has(Subject, Predicate, Object) :-
%         ((compound(Subject));(compound(Predicate));(compound(Object))),
%         ((compound(Subject))->
%           (rdf_global_id(Subject, SubjGlobal);(SubjGlobal=Subject) ); (SubjGlobal=Subject)),
%         ((compound(Predicate))->
%           (rdf_global_id(Predicate, PredGlobal);(PredGlobal=Predicate)); (PredGlobal=Predicate)),
%         ((compound(Object))->
%           (rdf_global_id(Object, ObjGlobal);(ObjGlobal=Object)); (ObjGlobal=Object)),
%         !, rdf_has(SubjGlobal, PredGlobal, ObjGlobal, _).
...
:- if( current_predicate(rdf_db:rdf_set_graph_source/3) ).
