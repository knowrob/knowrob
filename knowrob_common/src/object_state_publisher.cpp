...
#include <knowrob_common/ObjectState.h>
#include <knowrob_common/ObjectStateArray.h>

class MarkDirtyObjectClient {
private:
	typedef std::map<std::string,knowrob_common::ObjectState> ObjectStateMap;
	typedef std::pair<std::string,knowrob_common::ObjectState> ObjectStateMapItem;
...
public:
	...
	void broadcast(ros::Publisher &publisher) {
		...
		push_lock.unlock();
		if(!objects.empty()) {
			knowrob_common::ObjectStateArray msg;
			msg.action = knowrob_common::ObjectStateArray::ADD;
		...
		if(!deleted.empty()) {
			knowrob_common::ObjectStateArray msg;
			msg.action = knowrob_common::ObjectStateArray::DELETE;
			for(std::set<std::string>::iterator it = deleted.begin(); it != deleted.end(); ++it) {
				knowrob_common::ObjectState os_msg;
	...
	void loop() {
		ros::NodeHandle n_;
		ros::Publisher publisher_ = n_.advertise<knowrob_common::ObjectStateArray>("/object_state", 10000);
		while(ros::ok()) {
			{
	...
	void push(const PlTerm &data) {
		...
		ObjectStateMap::iterator it = dirty_objects[push_index].insert(
				ObjectStateMapItem(object_id,knowrob_common::ObjectState())).first;
		knowrob_common::ObjectState &obj = it->second;
