...
:- module('knowrob/reasoning/action_parser', [
     parser_create/1,
     parser_create/2,
     parser_run/3,
     parser_start/1,
...
:- use_module(library('knowrob/model/Resource'), [ kb_resource/1 ]).
:- use_module(library('knowrob/model/Plan'),     [ plan_defines_task/2 ]).

:- use_module(library('knowrob/comp/event_graph')).
:- use_module(library('knowrob/reasoning/task_planning'), [workflow_constituents/3]).
...
parser_start(Parser) :-
  ...
  ignore(ros_subscribe('/parser/token',
      'knowrob_common/EventToken',
      'knowrob/reasoning/action_parser':ros_push_token(Parser),
      Subscriber)),
...
is_token_(tok(Time,Endoint,Objects)) :-
  %% 1.ARG
  number(Time),
  %% 2.ARG
  endpoint_type(Endoint,EvtType),
  kb_resource(EvtType),
  %% 3.ARG
