...
test(query_refrigerator_by_name, [nondet]) :-
  entity(Cont, [an, object, [name, entity_test:'Refrigerator_fg45543']]),
  rdf_equal(Cont, entity_test:'Refrigerator_fg45543').
...
test(assert_temporalized) :-
  entity_assert(Obj, [an, object, [type, drawer], [volume_of_object, 10.0, during, [0.0,20.0]]]),
  atom(Obj), rdfs_individual_of(Obj, knowrob:'Drawer'),
  holds(knowrob:'volumeOfObject'(Obj, 10.0), 5.0),
  holds(knowrob:'volumeOfObject'(Obj, 10.0), [0.0,20.0]),
  not( holds(knowrob:'volumeOfObject'(Obj, 10.0), [0.0,21.0]) ).

test(assert_temporalized_volume) :-
  entity_assert(Obj, [an, object, [type, dough],
        [volume_of_object, 10.0, during, [0.0,20.0]],
        [volume_of_object, 15.0, during, [20.0,30.0]]]),
  rdfs_individual_of(Obj, knowrob:'Dough'),
  holds(knowrob:'volumeOfObject'(Obj, 10.0), 5.0),
  holds(knowrob:'volumeOfObject'(Obj, 10.0), [0.0,20.0]),
  not( holds(knowrob:'volumeOfObject'(Obj, 10.0), [0.0,21.0]) ),
  holds(knowrob:'volumeOfObject'(Obj, 15.0), 25.0),
  holds(knowrob:'volumeOfObject'(Obj, 15.0), [20.0,30.0]).

test(generate_temporalized_description) :-
  Descr=[an, object, [type, drawer], [volume_of_object, 10.0, during, [60,70]]],
