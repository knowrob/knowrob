...
:- use_module(library('ias_knowledge_base')).
...
:- owl_parser:owl_parse('../owl/ccrl_semantic_map.owl', false, false, true).
...
readEntity(E, Parent, [E, Parent, Pose]) :-
    objectPose( E, Pose ).

rdf_atom_no_ns( Tin, Tout ) :-
    rdf_global_id(_:Tout, Tin ).
