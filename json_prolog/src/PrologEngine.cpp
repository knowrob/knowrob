...
void PrologEngine::run()
{
	...
	ROS_DEBUG("[%s] json_prolog thread started.", thread_id.c_str());
	
	if(!PL_thread_attach_engine(NULL)) {
		ROS_ERROR("json_prolog failed to attach engine!");
		is_terminated_ = true;
		return;
	}
	//
	while(ros::ok()) {
		...
		ROS_DEBUG("[%s] json_prolog thread claimed.", thread_id.c_str());
		// create a query
		...
		qid_t qid = PL_open_query(NULL,
		    ...
		    PL_pred(PL_new_functor(PL_new_atom("json_prolog_query"),2),NULL),
		    a1);
		...
		ROS_DEBUG("[%s] json_prolog thread released.", thread_id.c_str());
	}
	
	ROS_DEBUG("[%s] json_prolog thread terminated.", thread_id.c_str());
	finished_cv_.notify_all();
	solutions_cv_.notify_all();
	is_terminated_ = true;
