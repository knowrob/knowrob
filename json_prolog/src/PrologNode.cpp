...
{
	ros::init(argc, argv, "json_prolog");
	ros::NodeHandle n;
	// json_prolog can serve requests in parallel
	...
	if(json_prolog.is_initialized()) {
		...
		ROS_INFO("json_prolog service is exiting.");
		
		return EXIT_SUCCESS;
	}
	else {
		ROS_ERROR("json_prolog service failed to initialize.");
		return EXIT_FAILURE;
	}
}
