...
TEST_F(AnswerCombinerTest, CombineMany_DifferentVariables) {
	...
	answer11->set(Variable("a"), std::make_shared<Integer32Term>(4));
	answer21->set(Variable("b"), std::make_shared<Integer32Term>(6));
	answer22->set(Variable("b"), std::make_shared<Integer32Term>(7));
...
TEST_F(AnswerCombinerTest, CombineMany_Unification) {
	...
	answer11->set(Variable("a"), QueryParser::parsePredicate("p(X,1)"));
	answer21->set(Variable("a"), QueryParser::parsePredicate("p(2,Y)"));
	answer22->set(Variable("a"), QueryParser::parsePredicate("p(2,2)"));
	input1->push(answer11);
	input2->push(answer21);
