...
: FormulaQueryStage(modal->modalFormula(), ctx),
  kb_(kb),
  modalFormula_(modal)
{
	...
	switch(modalOperator->modality().modalityType()) {
		case ModalityType::Epistemic: {
			...
			if(ctx->selector_.agent.has_value() && ctx->selector_.agent.value() != Agent::getEgo()) {
				...
				throw QueryError("epistemic formula {} is embedded within epistemic context of another agent.", *modalFormula_);
			}
...
AnswerBufferPtr ModalStage::submitQuery_K(const FormulaPtr &phi)
{
...
AnswerBufferPtr ModalStage::submitQuery_B(const FormulaPtr &phi)
{
...
AnswerBufferPtr ModalStage::submitQuery_P(const FormulaPtr &phi)
{
...
AnswerBufferPtr ModalStage::submitQuery_H(const FormulaPtr &phi)
{
...
AnswerBufferPtr ModalStage::submitQuery(const FormulaPtr &phi)
{
