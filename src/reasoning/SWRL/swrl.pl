...
:- rdf_meta swrl_match_instance(r,r,r).
...
%can_answer(is_instance_of(_,_)).
%can_answer(is_subclass_of(_,_)).
...
infer(Query,Fact,Scope) :-
  % TODO
  % TODO: could to tell other implications as well!
  fail.
...
%
swrl_assert(Rule) :- swrl_assert2(Rule,_).

swrl_assert2([] :- _, []) :- !.

swrl_assert2([HeadAtom|Xs] :- Body, [Asserted|Rest]) :-
...
%% swrl_implication_pl
swrl_implication_pl(class(Cls,S), swrl_holds(S_var,P,Cls_rdf,Scope), Vars) :-
  swrl_var(Vars, S, S_var),
  swrl_var(Vars, 'swrl:scope', Scope),
  rdf_equal(rdf:type, P),!,
  % FIXME rather transform to lang term
...
swrl_condition_pl(class(Cls,S),
        ask(is_instance_of(S_var,Cls_rdf),Scope),
        Vars) :-
  swrl_var(Vars, S, S_var),
  swrl_var(Vars, 'swrl:scope', Scope),
  % FIXME rather transform to lang term
...
swrl_var_names(Var, Var) :-
  atom(Var),
  % FIXME: what is this doing here? should be handled in rdf.pl, or?
...
% FIXME: expand to lang term, do not use literal(...)
...
swrl_satisfied([HeadAtom|Xs] :- Body, Vars_user, Vars) :-
  ...
  swrl_vars([HeadAtom|Xs] :- Body, Vars_rule), Vars = [var('swrl:scope',_{})|Vars_rule],
