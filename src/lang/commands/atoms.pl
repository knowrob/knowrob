...
%% query variables
query_compiler:step_var(atom_number(A,N), Var)      :- atom_var([A,N],Var).
query_compiler:step_var(atom_length(A,L), Var)      :- atom_var([A,L],Var).
query_compiler:step_var(atom_prefix(A,P), Var)      :- atom_var([A,P],Var).
query_compiler:step_var(atom_concat(A1,A2,A3), Var) :- atom_var([A1,A2,A3],Var).
query_compiler:step_var(atomic_list_concat(List,Sep,A), Var) :- atom_var([A,Sep|List],Var).
query_compiler:step_var(atomic_list_concat(List,A), Var)     :- atom_var([A|List],Var).
...
query_compiler:step_compile(
	...
	findall(Step,
		(	set_if_var(Atom,    ['$toString', Number0], Step)
		;	set_if_var(Number,  ['$toDouble', Atom0],   Step)
...
query_compiler:step_compile(
	...
	findall(Step,
		(	set_if_var(Length,  ['$strLenCP', Atom0],   Step)
		;	match_equals(Atom0, ['$toString', Length0], Step)
		),
		Pipeline).
...
query_compiler:step_compile(
	...
	findall(Step,
		...
		;	set_if_var(Atom,    ['$concat', array([Left0,Right0])], Step)
...
query_compiler:step_compile(
	...
	query_compiler:var_key_or_val(Atom, Atom0),
	findall(Step,
		;	set_if_var(Atom0,   ['$concat', array(List0)], Step)
...
query_compiler:step_compile(
	...
	findall(Step,
		;	set_if_var(Atom,    ['$concat', array(List1)], Step)
