...
computable_reasoner2_(Module,ComputableTerm) :-
  ...
  %
  forall(
    tripledb_subproperty_of(Property,X),
    ( % assert *can_answer* clause
      assert_can_answer_(Module,SubProperty),
      % assert *infer* clause
      assert_infer_(Module,SubProperty,Property,Predicate)
    )
  ).
...
%%
assert_infer_(M,P_parent,P_specific,Predicate) :-
  ( P_parent=P_specific -> X=infer ; X=infer2 ),
  Head=..[X,holds(S,P_parent,O),holds(S,P_specific,O),Scope],
