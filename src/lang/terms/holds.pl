...
:- use_module(library('model/OWL'),
        [ is_object_property/1, is_data_property/1 ]).
:- use_module(library('model/QUDT'),
        [ qudt_unit/4, qudt_conv/4 ]).
:- use_module(library('db/tripledb'),
        [ tripledb_ask/4, tripledb_tell/4 ]).
...
holds(S,P,O) ?>
  { var(P); is_object_property(P) },
  { strip_operator_(O,O1,=) },
  holds_object(S,P,O1).

holds(S,P,DataTerm) ?>
  { var(P); is_data_property(P) },
  ...
  { ground(QueryUnit) ->
    ...
    ( ValueQuery=..[QueryOperator,DataValue],
      DataValue=QueryValue )
  },
  `call`(holds_data(S,P,ValueQuery), _{unit: DataUnit}),
  % finally ground remaining vars in StrippedQuery
  { data_term_unify_(StrippedQuery,DataValue,DataUnit) }.

holds(S,P,O) +>
  { is_object_property(P), ! },
  % O1 should be the bare IRI atom
  { strip_operator_(O,O1,=), atom(O1) },
  tripledb_tell(S,P,O1).
...
holds(S,P,DataTerm) +>
  ...
  % TODO: is unit really scope? if so can it be done externally?
  `call`(holds_data(S,P,DataValue::BaseType), _{unit: DataUnit}).
...
%
data_term_unify_(Term,'=',DataValue,DataUnit) :-
  var(Term),!,
  ( ground(DataUnit) ->
    Term=..[DataUnit,DataValue];
    Term=DataValue
  ).

data_term_unify_(Term,DataValue,DataUnit) :-
  compound(Term),!,
  Term=..[QueryUnit,QueryValue],
  ( QueryUnit=DataUnit ->
    Actual=DataValue;
...
%
data_base_type_(_Property,DataValue,term) :-
  compound(DataValue),!.
...
data_base_type_(_Property,DataValue,number) :-
  number(DataValue),!.

data_base_type_(_Property,DataValue,string) :-
  atom(DataValue),!.
