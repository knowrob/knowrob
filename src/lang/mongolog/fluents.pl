...
Fluents change their value over time.
Stamped documents are used to represent the value
over time.
At any timepoint, the document with the latest stamp
before the timepoint provides fluent value.
