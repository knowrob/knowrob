:- begin_tests('knowrob/comp/event_graph').

:- use_module(library('knowrob/comp/event_graph')).

test('merge_sequences(equal0)') :-
  'knowrob/comp/event_graph':merge_sequences(
    [a,b],
    [a,b],
    [a,b]).
...
test('merge_sequences(prefix+suffix_2)') :-
  'knowrob/comp/event_graph':merge_sequences2([
    [a,     b,c,   z],
    [a,      d,    z]],
    [a,[[b,c],[d]],z]).
...
test('path_to_endpoint(-b)') :-
  'knowrob/comp/event_graph':path_to_endpoint(
    [[-a,-z],[+a],[-b],[+b,+z]],
    -b,
    [[-a,-z],[+a]],
    [[-b],[+b,+z]]).
...
:- end_tests('knowrob/comp/event_graph').
